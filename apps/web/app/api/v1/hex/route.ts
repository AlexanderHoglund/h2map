  ...
  try {
    ...
    return Response.json({
      cells: (data ?? []).map((row) => ({
        ...
        // has visited yet. Read defensively: the generated types are
        // regenerated by the operator after migration 20260815000001, so
        // until then the columns exist in SQL but not in the type.
        pvDbTier: (row as Record<string, unknown>).pv_db_tier ?? null,
        windFidelity: (row as Record<string, unknown>).wind_fidelity ?? null,
      })),
    });
  } catch (err) {
